...
app.layout = html.Div([
    ...
    # Fake progress bar — fills 0→100% while the real callback runs
    html.Div([
        html.Div(id="progress-fill", style={
            'height': '6px', 'width': '0%', 'backgroundColor': '#2563eb',
            'borderRadius': '999px', 'transition': 'width 0.2s ease'
        })
    ], id="progress-track", style={
        'height': '6px', 'backgroundColor': '#e2e8f0', 'borderRadius': '999px',
        'maxWidth': '400px', 'margin': '0 auto 20px', 'overflow': 'hidden', 'display': 'none'
    }),

    dcc.Interval(id="progress-interval", interval=200, disabled=True),
    ...
    dcc.Loading(
        ...
        children=html.Div(
            id="dashboard-body",
            style={'minHeight': '500px'},   # <- stops the blank collapse while switching
            children=html.Div([
                html.Div("🌍", style={'fontSize': '48px', 'textAlign': 'center', 'marginBottom': '10px'}),
                html.P("Search a city or enter coordinates, then click Update Dashboard.",
                       style={'textAlign': 'center', 'color': '#94a3b8', 'fontSize': '15px'})
...
# Kick off the fake progress bar the moment the button is clicked
app.clientside_callback(
    ...
    function(n_clicks) {
        ...
        return [0, false, {'display': 'block'}];
    }
    """,
    Output("progress-value", "data", allow_duplicate=True),
    Output("progress-interval", "disabled", allow_duplicate=True),
    Output("progress-track", "style", allow_duplicate=True),
    Input("submit-btn", "n_clicks"),
    prevent_initial_call=True
)

# Tick the fake progress up while waiting (caps at 90% until real data arrives)
@app.callback(
...
def tick_progress(n_intervals, current_value):
    new_value = min(current_value + 12, 90)
    fill_style = {
        'height': '6px', 'width': f'{new_value}%', 'backgroundColor': '#2563eb',
        'borderRadius': '999px', 'transition': 'width 0.2s ease'
    }
    return new_value, fill_style
...
def update_dashboard(n_clicks, city_value, city_options, lat, lon):
    ...
    if lat is None or lon is None or not (-90 <= lat <= 90) or not (-180 <= lon <= 180):
        return "Please enter a valid latitude/longitude or select a city.", html.Div(), lat, lon, True, hide_bar
    if not city_label:
    ...
    return label, body, lat, lon, True, hide_bar
